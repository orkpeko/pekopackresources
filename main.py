...
print("Connecting to api.orkpeko.xyz...")
# TK Window define
window = Tk()
...
def clickdownloadmodso():
    ...
    r = requests.get('https://cdn.orkpeko.xyz/r/latestmo.zip', allow_redirects=True)
...
def clickdownloadmodss():
    ...
    for filename in os.listdir(folder):
        ...
        try:
            ...
            print('Failed to delete %s. Reason: %s' % (file_path, e))
    print('Sending request to cdn.orkpeko.xyz for the mod pack...')
    r = requests.get('https://cdn.orkpeko.xyz/r/latestms.zip', allow_redirects=True)
    open(f"{diren.get()}\\mods\\modste.zip", 'wb').write(r.content)
    with zipfile.ZipFile(f"{diren.get()}\\mods\\modste.zip", 'r') as zip_ref:
        zip_ref.extractall(f"{diren.get()}\\mods",)
    print("Done.")
